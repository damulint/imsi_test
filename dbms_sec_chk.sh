...
{
  echo "============================================================"
  echo "KISA DBMS UNIX Audit Framework v5"
  echo "============================================================"
  echo "[SERVER PROFILE]"
...
append_raw "execution note" "v5: client auto-detect, connection check, MySQL blank DBUSER support, socket fallback, safer CUBRID password masking, PostgreSQL hba_file auto-detect, Oracle/Tibero PUBLIC grants, expanded audit evidence"
...
case "$DBMS" in
  oracle)
    ...
    RAW11="$(exec_sql_label "D-11 oracle system table grants" "select grantee||','||privilege||','||owner||','||table_name from dba_tab_privs where (owner='SYS' or table_name like 'DBA_%') and grantee not in ('SYS','SYSTEM','DBA','PUBLIC') order by grantee, owner, table_name, privilege;" | head -300)"
    RAW12="$(
      if [ -n "${ORACLE_HOME:-}" ]; then
    ...
    RAW19="$(exec_sql_label "D-19 oracle os roles" "select name||','||value from v\$parameter where name in ('os_roles','remote_os_authent','remote_os_roles','resource_limit');")"
  ...
  mysql)
    ...
    if [ "$MYSQL_FLAVOR" = "mariadb" ]; then
      RAW01="$(exec_sql_label "D-01 mariadb privileged/default users" "SELECT User,Host,COALESCE(JSON_UNQUOTE(JSON_EXTRACT(Priv,'$.plugin')),''),COALESCE(JSON_UNQUOTE(JSON_EXTRACT(Priv,'$.account_locked')),''),COALESCE(JSON_UNQUOTE(JSON_EXTRACT(Priv,'$.password_last_changed')),'') FROM mysql.global_priv WHERE User in ('root','mysql','mariadb.sys','mysql.sys','mysql.session') OR User='' ORDER BY User,Host;")"
      [ -n "$RAW01" ] || RAW01="$(exec_sql_label "D-01 mariadb mysql.user fallback" "select user,host,plugin from mysql.user where user in ('root','mysql','mariadb.sys','mysql.sys','mysql.session') or user='' order by user,host;")"
      ...
      RAW01="$(exec_sql_label "D-01 mysql privileged/default users" "select user,host,plugin,account_locked,password_last_changed from mysql.user where user in ('root','mysql.sys','mysql.session') or user='' order by user,host;")"
    ...
    RAW14="$(
      ...
      collect_file_perm /var/lib/mysql /var/log/mysql /var/log/mariadb 2>/dev/null
    )"
...
${RAW20_PRIVS}"
  ...
  postgresql)
    ...
    RAW14="$(
      ...
      if [ -f "$PG_HBA_FILE" ]; then
        ...
        echo "[pg_hba.conf risky trust/all lines]"
  ...
  cubrid)
    ...
    RAW02="$RAW01"
    RAW04="$(exec_sql_label "D-04 cubrid users masked" "SELECT name, CASE WHEN password IS NULL OR password = '' THEN 'EMPTY_OR_NULL' ELSE 'SET' END AS password_status FROM db_user;")"
...
if [ "$DBMS" = "mysql" ] && printf "%s" "$RAW01$RAW02" | awk '{print $1}' | grep -qx ""; then
  D01_RESULT="VULN"
  D01_VERDICT="Vulnerable"
  D01_CURRENT="anonymous 계정 또는 기본 계정 위험 징후 발견"
fi
...
if [ -n "$ROOT_DB_PROC" ]; then
  write_item "D-07" "중" "root 권한으로 서비스 구동 제한" "VULN" "Vulnerable" "DBMS 프로세스가 root 권한으로 구동 중" "$ROOT_DB_PROC" "DBMS 전용 OS 계정으로 서비스 구동"
else
  write_item "D-07" "중" "root 권한으로 서비스 구동 제한" "OK" "Good" "root 권한 DBMS 프로세스 미발견" "process scan" "전용 계정 구동 상태 유지"
fi
...
write_item "D-10" "상" "원격 접속 제한" "$D10_RESULT" "$D10_VERDICT" "$D10_CURRENT" "${RAW10:-No data}
${RAW14:-}" "허용 IP, listener binding, pg_hba.conf, 계정 host 제한 점검"
