...
export LANG%m%d%H%M%S')"export LANG=C
...
is_loose_file_perm() {
  ...
  # File 기준:
  # - others 권한이 하나라도 있으면 취약 가능성
  # - group write 가능하면 취약 가능성
...
is_loose_dir_perm() {
  ...
  # Directory 기준:
  # - others write 가능하면 취약 가능성
  # - group write 가능하면 취약 가능성
  # 일반적인 755 디렉터리는 여기서 취약으로 보지 않음
...
# Directory Listing 오탐 감소:
# 단순 Indexing 문자열만으로 취약 처리하지 않고,
# On/Yes/Enable/True/Indexes 등 활성화 가능성이 높은 표현 위주로 탐지
listings="$(
  printf "%s\n" "$CONFIG_RAW" |
...
# WebDAV 오탐 감소:
# dav, put, delete 같은 일반 문자열을 무작정 잡지 않고,
# WebDAV 명령/HTTP Method 형태를 중심으로 탐지
webdav="$(
  printf "%s\n" "$CONFIG_RAW" |
...
# SSL/TLS 오탐 감소:
# SSL 관련 설정이 있다고 무조건 OK가 아니라 취약 프로토콜/암호군 여부 확인
weak_tls="$(
  printf "%s\n" "$CONFIG_RAW" |
...
if [ -n "$proxy" ]; then
  write_item "WEB-10" "High" "Restrict proxy settings" "WARN" "ManualCheck" "WebtoB" \
    "Proxy-like config found" \
    "$proxy" \
    "Keep only required proxy settings"
else
  write_item "WEB-10" "High" "Restrict proxy settings" "WARN" "ManualCheck" "WebtoB" \
    "Proxy config not auto-identified" \
    "$CONFIG_FILES" \
    "Verify proxy config"
fi

write_item "WEB-11" "Medium" "Proper web root path" "WARN" "ManualCheck" "WebtoB" \
  "Need document root review" \
  "$docroot" \
  "Verify dedicated deployment paths"

write_item "WEB-12" "Medium" "Avoid symbolic links" "WARN" "ManualCheck" "WebtoB" \
  "Need symlink/alias mapping review" \
  "$symlink_cfg" \
  "Verify link-like mapping"

write_item "WEB-13" "High" "Prevent config file exposure" "WARN" "ManualCheck" "WebtoB" \
  "Need deployed file review" \
  "$WEBTOB_HOME" \
  "Check config/backup files under service roots"

###############################################################################
# Permission check using separated file/dir logic
###############################################################################
...
if [ -n "$perm_vuln" ]; then
  write_item "WEB-14" "High" "Access control for files" "VULN" "Vulnerable" "WebtoB" \
    "Loose permissions found" \
    "$(printf "%b" "$perm_vuln")" \
    "Tighten permissions"
else
  write_item "WEB-14" "High" "Access control for files" "OK" "Good" "WebtoB" \
    "No obvious loose permission found by separated file/dir criteria" \
    "$WEBTOB_HOME" \
    "Keep current state"
fi

write_item "WEB-15" "High" "Remove unnecessary script mapping" "WARN" "ManualCheck" "WebtoB" \
  "Application/script mapping review required" \
  "$scriptmap" \
  "Review unnecessary handlers and mappings"

if [ -n "$headerhide" ]; then
  write_item "WEB-16" "Medium" "Limit header disclosure" "WARN" "ManualCheck" "WebtoB" \
    "Header/version config found" \
    "$headerhide" \
    "Verify actual response header disclosure"
else
  write_item "WEB-16" "Medium" "Limit header disclosure" "WARN" "ManualCheck" "WebtoB" \
    "Need live header verification" \
    "$CONFIG_FILES" \
    "Check actual response headers"
fi

write_item "WEB-17" "Medium" "Remove unnecessary virtual dirs" "WARN" "ManualCheck" "WebtoB" \
  "Virtual path review required" \
  "$vdir" \
  "Remove unnecessary virtual paths"

if [ -n "$webdav" ]; then
  write_item "WEB-18" "High" "Disable WebDAV" "VULN" "Vulnerable" "WebtoB" \
    "WebDAV-like config found" \
    "$webdav" \
    "Disable WebDAV/authoring methods"
else
  write_item "WEB-18" "High" "Disable WebDAV" "OK" "Good" "WebtoB" \
    "WebDAV enabled-like config not found" \
    "$CONFIG_FILES" \
    "Keep current state or verify if application uses authoring methods"
fi
...
if [ -n "$ssl_disabled" ]; then
  write_item "WEB-20" "High" "Enable SSL/TLS" "VULN" "Vulnerable" "WebtoB" \
    "SSL/TLS appears disabled" \
    "$ssl_disabled" \
    "Enable HTTPS and use modern TLS settings"
elif [ -n "$weak_tls" ]; then
  write_item "WEB-20" "High" "Enable SSL/TLS" "VULN" "Vulnerable" "WebtoB" \
    "Weak SSL/TLS setting found" \
    "$weak_tls" \
    "Disable SSLv2/SSLv3/TLS1.0/TLS1.1 and weak ciphers"
elif [ -n "$ssl" ]; then
  write_item "WEB-20" "High" "Enable SSL/TLS" "WARN" "ManualCheck" "WebtoB" \
    "SSL/TLS config found but strength is not fully verified by static check" \
    "$ssl" \
    "Verify TLS protocol and cipher suites with live scan or vendor config review"
else
  write_item "WEB-20" "High" "Enable SSL/TLS" "VULN" "Vulnerable" "WebtoB" \
    "HTTPS/SSL config not found" \
    "$CONFIG_FILES" \
    "Enable HTTPS"
fi
...
if [ -n "$redirect_https" ]; then
  ...
  write_item "WEB-21" "Medium" "HTTP redirect to HTTPS" "WARN" "ManualCheck" "WebtoB" \
    "HTTPS redirect not auto-confirmed" \
    "$CONFIG_FILES" \
    "Verify redirect"
fi
...
if [ -n "$ldap" ]; then
  write_item "WEB-23" "Medium" "LDAP algorithm configuration" "WARN" "ManualCheck" "WebtoB" \
    "LDAP/digest config found" \
    "$ldap" \
    "Verify SHA-256+ and TLS"
else
  write_item "WEB-23" "Medium" "LDAP algorithm configuration" "WARN" "ManualCheck" "WebtoB" \
    "LDAP use not confirmed" \
    "$CONFIG_FILES" \
    "Verify only if LDAP is used"
fi

if [ -n "$uploads" ]; then
  write_item "WEB-24" "Medium" "Separate upload path and ACL" "WARN" "ManualCheck" "WebtoB" \
    "Upload-like directories found" \
    "$uploads" \
    "Verify non-webroot placement and ACL"
else
  write_item "WEB-24" "Medium" "Separate upload path and ACL" "WARN" "ManualCheck" "WebtoB" \
    "Upload path not auto-identified" \
    "App-specific" \
    "Verify upload segregation manually"
fi

write_item "WEB-25" "High" "Apply security patches" "WARN" "ManualCheck" "WebtoB" \
  "Installed version identified" \
  "$WEBTOB_VER" \
  "Compare with vendor advisories"
...
if [ -n "$logdir" ] && is_loose_dir_perm "$logdir"; then
  write_item "WEB-26" "Medium" "Log directory/file permission" "VULN" "Vulnerable" "WebtoB" \
    "Loose log directory permission" \
    "$logdir $(stat -c '%U:%G %a' "$logdir" 2>/dev/null)" \
    "Tighten log permissions"
else
  write_item "WEB-26" "Medium" "Log directory/file permission" "WARN" "ManualCheck" "WebtoB" \
    "Need WebtoB log dir review" \
    "$logdir" \
    "Verify actual log path and ACL"
fi
